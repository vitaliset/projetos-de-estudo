...
# https://medium.com/datalab-log/voc%C3%AA-deve-calibrar-seu-modelo-desbalanceado-ou-n%C3%A3o-3f111160653a
def calib_curve_proportions(y_true, y_probs, n_bins=10, method="uniform"):
    ...
    if method == "quantile":
        quantiles = [
            predictions["scores"].quantile(q) for q in np.linspace(0, 1, n_bins + 1)
        ]
    ...
    p = predictions.groupby("bins").apply(lambda x: len(x)).values
    p = p / len(y_true)
